...
(function() {
  ...
  TWTR.Widget = function(opts) {
    this.init(opts);
    // Activa el input de búsqueda (si hubiera)
    this.initSearch();
  };
  ...
  (function() {
    ...
    var absoluteTime = function(s) {
      var d = new Date(s);
      // nelson removed this 17.10.2011 - break on IE
      //if (browser.ie) { d = Date.parse(s.replace(/( \+)/, ' UTC$1')); }
      var ampm = '';
      var hour = function() {
        var h = d.getHours();
        if (h > 0 && h < 13) {
    ...
    var timeAgo = function(dateString) {
      ...
      if (diff < minute) {
        return "hace cerca de " + Math.floor(diff / second) + " segundos";
      }
      ...
      if (diff < hour) {
        return "hace cerca de " + Math.floor(diff / minute) + " minutos";
      }

      if (diff < hour * 2) {
        return "hace cerca de 1 hora";
      }

      if (diff < day) {
        return  "hace cerca de " + Math.floor(diff / hour) + " horas";
      }
    ...
    function Tweet(tweet) {
      function showPopular() {
        if (tweet.needle.metadata && tweet.needle.metadata.result_type && tweet.needle.metadata.result_type == 'popular') {
          return '<span class="twtr-popular">' + tweet.needle.metadata.recent_retweets + '+ retweets recientes</span>';
        } else {
          return '';
        }
      }
      ...
      var html = '<div class="twtr-tweet-wrap"> \
        ...
        <div class="twtr-tweet-text"> \
          ...
          <a target="_blank" class="twtr-reply" href="http://twitter.com/?status=@' + tweet.user + '%20&in_reply_to_status_id=' + tweet.id + '&in_reply_to=' + tweet.user + '">responder</a> \
          </em> ' + showPopular() + ' \
        </div> \
      </div>';
    ...
    TWTR.Widget.prototype = function() {
      ...
      return {
        init: function(opts) {
          ...
          this.tweet_creator = Tweet;

          this.set_tweet_creator = function(f) {
            this.original_creator = this.tweet_creator;
            this.tweet_creator = f;
          };
          ...
          if (opts.tweet_creator) {
            that.tweet_creator = opts.tweet_creator;
          }
        ...
        // Activa el input de búsqueda (si hubiera)
        initSearch: function(){
            var parent = this.widgetEl.parentNode;
            while (parent.className && parent.className.split(' ')[0] != 'twitter_module'){
        ...
        _setUrl: function() {
          ...
          if (this._isProfileWidget) {
            this.url = this._base + '&callback=' + this._cb +
                       '&include_rts=true' +
                       '&count=' + this.rpp + showSince() + '&clientsource=' + this.source + '&lang=' + this.language;
          }
        ...
        _createTweet: function(o) {
          ...
          this.tweet = new this.tweet_creator(o); //Tweet(o);
        ...
        _insertNewResultsNumber: function() {
          ...
          if (this.runOnce && this._isSearchWidget) {
            ...
            var plural = newResultsTotal > 1 ? 'nuevos tweets' : 'tweet nuevo';
            var moreThan = (this.response.warning && this.response.warning.match(/adjusted since_id/)) ? '' : 'más de';
            var el = document.createElement('div');
            classes.add(el, 'twtr-new-results');
            el.innerHTML = '<div class="twtr-results-inner"> &nbsp; </div>' +
                           '<div class="twtr-results-hr"> &nbsp; </div><span>' + moreThan + ' <strong>' + newResultsTotal + '</strong> ' + plural + '</span>';
            insertAfter(el, this.byClass('twtr-reference-tweet', 'div'));
            this._hasNewSearchResults = false;
          }
        },
